...
angular.module('avaughan.user', [
  'avaughan.logging',
  'avaughan.login'
]);
angular.module('avaughan.user').provider('avUserService', function () {
  ...
  this.initialize = function (userResourceUrl, userNameVariable, urlAfterLogin, mockData) {
    console.log('avaughan.user initialize called', [
      userResourceUrl,
      urlAfterLogin,
      mockData
    ]);
    ...
    function ($rootScope, $resource, avLog, $location, avLogin, $cookieStore, $http) {
      ...
      serviceContainerConfig.mockExtend = {
        ...
        login: function (username, password) {
          this.logger.debug('in mock mode, login', [
            username,
            password
          ]);
      ...
      serviceContainerConfig.serviceExtend = {
        ...
        customInitialize: function (dependencies, requirements) {
          var self = this;
          this.logger.info('custom initialize hooking up auth events', [
            dependencies,
            requirements
          ]);
    ...
    return {
      link: function (scope, element, attrs) {
        ...
        $rootScope.$on('SERVICE.' + avUserService.eventChannel + '.GET.SUCCESS', function (event, user) {
          logger.info('SERVICE.' + avUserService.eventChannel + '.GET.SUCCESS user service got session login event ', [
            event,
            user
          ]);
